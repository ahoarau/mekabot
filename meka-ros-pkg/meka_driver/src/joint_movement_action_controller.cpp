                                  ...
                                  boost::bind(&JointMovementActionController::executeCB, this, _1), false)
{
  ...
  gripper_joints_ = meka_->getGripperJointNames();
                                                                                             ...
                                                                                             const sensor_msgs::JointState &jointGoal)
{
  ...
  {
    ROS_DEBUG("%s - min: %f - max: %f - curr: % f - req: %f", jointGoal.name[i].c_str(), meka_->getMotorLimitMin(jointGoal.name[i]), meka_->getMotorLimitMax(jointGoal.name[i]), meka_->getMotorAngles()[meka_->getJointIndex(jointGoal.name[i])], jointGoal.position[i]);
  }

  for (size_t i = 0; i < jointGoal.name.size(); i++)
  {

    if (jointGoal.position[i] < meka_->getMotorLimitMin(jointGoal.name[i]))
    {
      adjustedJointGoal.position[i] = meka_->getMotorLimitMin(jointGoal.name[i]);

      ROS_INFO("%s - requested JointGoalPosition: %f exceeded MotorLimit: %f  - adjusted JointGoalPosition to MotorLimit", jointGoal.name[i].c_str(), jointGoal.position[i], meka_->getMotorLimitMin(jointGoal.name[i]));
    }

    if (jointGoal.position[i] > meka_->getMotorLimitMax(jointGoal.name[i]))
    {
      adjustedJointGoal.position[i] = meka_->getMotorLimitMax(jointGoal.name[i]);
      ROS_INFO("%s - requested JointGoalPosition: %f exceeded MotorLimit: %f  - adjusted JointGoalPosition to MotorLimit", jointGoal.name[i].c_str(), jointGoal.position[i], meka_->getMotorLimitMax(jointGoal.name[i]));
    }
  }
