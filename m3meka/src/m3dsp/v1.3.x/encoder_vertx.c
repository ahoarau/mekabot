...
int vertx_idx;
unsigned int volatile vertx_buffer[NUM_VERTX_CH][VERTX_NUM_SMOOTH];
//unsigned int volatile vertx_popcorn_buffer[NUM_VERTX_CH][3];
unsigned int volatile vertx_error_buffer[NUM_VERTX_CH];
//int vertx_popcorn_idx;
...
{
	unsigned int a=vertx_popcorn_buffer[ch][0];
	unsigned int b=vertx_popcorn_buffer[ch][1];
	unsigned int c=vertx_popcorn_buffer[ch][2];
	if (((a<=b)&&(a>=c))||((a<=c)&&(a>=b)))
		return a;
	if (((b<=a)&&(b>=c))||((b<=c)&&(b>=a)))
		return b;
...
void setup_vertx(void)
{
	...
	BB_SPI_SS_SET
#else
	BB_SPI_SEL=1; // for older DAC board
#endif
...
//Handle both channel in parallel
	...
	//vertx_popcorn_buffer[VERTX_CH_A][vertx_popcorn_idx]=vertx_val[VERTX_CH_A];
	//vertx_popcorn_buffer[VERTX_CH_B][vertx_popcorn_idx]=vertx_val[VERTX_CH_B];
#endif
	...
	//vertx_popcorn_idx=INC_MOD(vertx_popcorn_idx,3);
}
#endif
