...
JointTrajectoryActionController::JointTrajectoryActionController(boost::shared_ptr<AbstractMeka> meka) :
  meka_(meka), action_server_(ros::NodeHandle(), "meka_arm_controller/joint_trajectory_action",
                                  boost::bind(&JointTrajectoryActionController::executeCB, this, _1), false),
  action_server_follow_(ros::NodeHandle(), "meka_arm_controller/follow_joint_trajectory",
			boost::bind(&JointTrajectoryActionController::executeCBFollow, this, _1), false)
{
  ...
  sub_command_ = node_.subscribe("meka_arm_controller/command", 1, &JointTrajectoryActionController::commandCB, this);
  serve_query_state_ = node_.advertiseService("meka_arm_controller/query_state", &JointTrajectoryActionController::queryStateService, this);
  controller_state_publisher_ = node_.advertise<pr2_controllers_msgs::JointTrajectoryControllerState> ("meka_arm_controller/state", 1);
...
void JointTrajectoryActionController::reset_trajectory_and_stop()
{
  ...
  for (size_t j = 0; j < joints_.size(); ++j)
    hold[0].splines[j].coef[0] = meka_->getMotorAngles()[j];
...
void JointTrajectoryActionController::update()
{
  ...
  for (size_t i = 0; i < joints_.size(); ++i)
  {
    error[i] = meka_->getMotorAngles()[i] - q[i];
  }
  ...
  for (size_t j = 0; j < joints_.size(); ++j)
  {
    ...
    msg.actual.positions[j] = meka_->getMotorAngles()[j];
    msg.actual.velocities[j] = meka_->getMotorVelocities()[j];
    // ignoring accelerations
    msg.error.positions[j] = error[j];
    msg.error.velocities[j] = meka_->getMotorVelocities()[j] - qd[j];
    // ignoring accelerations
  }
...
void JointTrajectoryActionController::commandCB(const trajectory_msgs::JointTrajectory::ConstPtr &msg)
{
  ...
  JTAC action_client("meka_arm_controller/joint_trajectory_action", true);
                                                   ...
                                                   boost::function<bool()> isPreemptRequested)
{
  ...
  ROS_INFO("Sending trajectory to Meka arm...");
  bool success = meka_->executeTrajectory(new_traj);
  if (!success)
  {
    ROS_ERROR("Problem while transferring trajectory to Meka arm, aborting");
  ...
  while (ros::ok())
  {
    ...
    meka_->refreshMotorStatus();

    if (meka_->someMotorCrashed())
    {
    ...
    // all joints are idle
    if (meka_->allJointsReady() && allJointsStopped())
    {
...
bool JointTrajectoryActionController::goalReached()
{
  for (size_t i = 0; i < joints_.size(); i++)
  {
    double error = current_trajectory_->back().splines[i].target_position - meka_->getMotorAngles()[i];
...
bool JointTrajectoryActionController::allJointsStopped()
{
  for (size_t i = 0; i < joints_.size(); i++)
  {
    ...
    if (fabs(meka_->getMotorVelocities()[i]) > stopped_velocity_tolerance_)
      return false;
  }
  return true;
}
...
bool JointTrajectoryActionController::validTrajectory(const SpecifiedTrajectory &traj)
{
  ...
  {
    if (std::abs(traj[0].splines[j].coef[0] - meka_->getMotorAngles()[j]) > POSITION_TOLERANCE)
    {
      ROS_ERROR("Initial joint angle of trajectory (%f) does not match current joint angle (%f) (joint %zu)", traj[0].splines[j].coef[0], meka_->getMotorAngles()[j], j);
      return false;
    }
  }
