...
void isr_update_input_pdo(void)
{
	...
	//ec_stat.status[0].qei_period=vertx_error(VERTX_CH_SEAS);
        ...
        //ec_stat.status[0].adc_torque=get_avg_adc_spi(0);

        ec_stat.status[0].adc_torque=get_sum_adc_spi(0);
	...
	*/
	...
	#ifdef USE_ADC
        if (ec_cmd.command[0].config & ACTUATOR_EC_CONFIG_HAS_BRAKE == ACTUATOR_EC_CONFIG_HAS_BRAKE )
            ec_stat.status[0].adc_ext_temp = 0;
        else
